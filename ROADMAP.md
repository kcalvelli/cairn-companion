...
Tier 0, Tier 1, and Tier 2 are all live as of 2026-04-17. The project is in a stable state — feature work is slowing in favor of soak time, bug fixes, and follow-on polish.

- **Tier 0 (shell wrapper)** — shipped, in daily use.
- **Tier 1 (daemon + surfaces)** — daemon, OpenAI gateway, TUI dashboard, and all four channel adapters (Telegram, XMPP, email, Discord) are shipped. `cli-client` core subcommands are shipped; a handful of subcommands stay deferred by design.
- **Tier 2 (distributed agency)** — `spoke-tools`, `memory-tier`, and `spoke-http-transport` are shipped. Each machine runs its own gateway + spoke tools; memory syncs via Syncthing; remote spokes are HTTP MCP servers consumed directly. The original `distributed-routing` hub/spoke proposal is superseded and kept only for historical reference.

Open work: `cli-client` deferred subcommands, `tui-session-management`, `cairn-integration` (lives in the cairn repo). Nothing on this list is blocking — the project is shippable as-is. `companion-doctor` shipped 2026-06-10 (the first item from the post-v0.1 review queue in [IMPROVEMENTS.md](./IMPROVEMENTS.md)).
...
- [x] **[tui-dashboard](./openspec/changes/archive/tui-dashboard/)** — Terminal-native dashboard built on ratatui. Shipped 2026-04-07. Sessions panel, live streaming conversation view, status bar, vim-style navigation, auto-reconnect. Deferred: memory/events/usage panels (need new daemon methods). *Depends on: bootstrap, daemon-core*
- [ ] **tui-session-management** — Session management in the TUI: list sessions by surface, view last activity, delete stale sessions. Motivated by a stale OpenAI gateway session that caused exit-1 failures after a model retirement — took 20 minutes of log diving to find what `sqlite3 sessions.db DELETE` would have fixed in 5 seconds. *Depends on: tui-dashboard, daemon-core*
- [x] **[channel-telegram](./openspec/changes/archive/channel-telegram/)** — First channel adapter; establishes the pattern for subsequent adapters. Shipped 2026-04-07. teloxide long-poll, allowlist, edit-in-place streaming, 4096-char splitting, /new /status /help commands, typing indicator. Deployed on mini only (one bot token = one poller). *Depends on: bootstrap, daemon-core*
...
- [x] **[companion-doctor](./openspec/changes/archive/2026-06-10-companion-doctor/)** — `companion doctor` health check across every surface. Shipped 2026-06-10. Daemon liveness, session store, per-channel adapter state (new `GetHealth` D-Bus method + a `HealthRegistry` the adapters write to), gateway `/health`, every spoke in `spoke-servers.json`, workspace, and persona-file resolution, in one color-coded report with `--json`. Exits nonzero only on real `FAIL` — daemon-independent checks run even when the daemon is down, and session-scoped spokes (the `After=graphical-session.target` set) report `WARN` not `FAIL` when unreachable so a headless fleet host doesn't peg the exit code. 159 daemon + 10 CLI tests, verified live on edge + mini. *Depends on: daemon-core, cli-client.*
- [x] **[model-overrides](./openspec/changes/model-overrides/)** — Per-surface Claude model selection via Nix. Shipped 2026-05-28. `services.cairn-companion.daemon.defaultModel`, `channels.{email,discord,telegram,xmpp}.model`, and `gateway.openai.backendModel` lower to `COMPANION_MODEL_*` env vars; dispatcher resolves explicit-caller-override > per-surface > daemon-default > claude-code's own default before forming `--model` on the subprocess argv. Distinct from the cosmetic gateway `modelName` (the latter is what `/v1/models` echoes; the former is what actually runs). Zero behavior change when every option is left at `null`. *Depends on: daemon-core.*
