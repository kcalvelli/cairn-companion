...
- [ ] **[spoke-http-transport](./openspec/changes/spoke-http-transport/)** — HTTP transport mode for spoke tools, enabling remote spokes on other machines in the fleet. Edge's mcp-gateway connects to remote spokes as HTTP transport entries with host-prefixed server IDs (`mini-companion-shell`). No mcp-gateway changes needed — just an HTTP wrapper around the existing `ToolHandler` trait + Nix wiring for systemd services. This is the final piece: after this, companion is feature-complete. *Depends on: spoke-tools.*
